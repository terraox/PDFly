  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">Transactions & Revenue</h2>
          <p className="text-zinc-400 text-sm">Monitor incoming payments and handle refunds.</p>
        </div>
        <button className="flex items-center gap-2 h-10 rounded-md bg-zinc-800 px-4 text-sm font-medium text-zinc-200 hover:bg-zinc-700 transition-colors border border-zinc-700">
      ...
      <div className="grid gap-6 md:grid-cols-4 mb-8">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <div className="text-zinc-400 text-xs font-bold uppercase tracking-wider">Gross Revenue</div>
          <div className="text-2xl font-bold text-white mt-2 flex items-center">
            <IndianRupee className="h-5 w-5 mr-1" /> 45,231
          </div>
          <span className="text-xs text-emerald-400 flex items-center mt-1"><ArrowUpRight className="h-3 w-3 mr-1" /> 12% vs last month</span>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <div className="text-zinc-400 text-xs font-bold uppercase tracking-wider">Net Revenue</div>
          <div className="text-2xl font-bold text-white mt-2 flex items-center">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <div className="text-zinc-400 text-xs font-bold uppercase tracking-wider">Failed Payments</div>
          <div className="text-2xl font-bold text-white mt-2">12</div>
          <span className="text-xs text-red-400 flex items-center mt-1"><ArrowDownRight className="h-3 w-3 mr-1" /> Action required</span>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <div className="text-zinc-400 text-xs font-bold uppercase tracking-wider">Avg Order Value</div>
          <div className="text-2xl font-bold text-white mt-2 flex items-center">
          ...
          <tbody className="divide-y divide-zinc-800">
            {TRANSACTIONS.map((tx) => (
              <tr key={tx.id} className="group hover:bg-zinc-800/50 transition-colors">
                ...
                <td className="px-6 py-4 text-zinc-300">{tx.user}</td>
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium ${
                    tx.status === 'SUCCESS' ? 'text-emerald-400 bg-emerald-400/10' : 
                    tx.status === 'FAILED' ? 'text-red-400 bg-red-400/10' : 
                    'text-zinc-400 bg-zinc-400/10'
                  }`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${
                      tx.status === 'SUCCESS' ? 'bg-emerald-400' : 
                      tx.status === 'FAILED' ? 'bg-red-400' : 'bg-zinc-400'
                    }`} />
                    {tx.status}
                  </span>
                </td>
