...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const savedExpiry = localStorage.getItem('pdfly_plan_expiry'); // NEW
    
    if (savedToken && savedEmail) {
      setToken(savedToken);
      setUser({ email: savedEmail, role: savedRole, planExpiry: savedExpiry }); // NEW
    }
  ...
  const login = (authToken, userEmail, userRole, planExpiry) => { // ADDED planExpiry
    ...
    localStorage.setItem('pdfly_plan_expiry', planExpiry); // NEW
    setToken(authToken);
    setUser({ email: userEmail, role: userRole, planExpiry: planExpiry }); // NEW
  };
  ...
  const logout = () => {
    ...
    localStorage.removeItem('pdfly_plan_expiry'); // NEW
    setToken(null);
    setUser(null);
  };
